...
export function investigationReport(item: Investigation, nonce: string): string {
  ...
  if (bounds) {
    ...
    const rings = parcel.geometry.coordinates.map(ring => ring.map(([longitude, latitude], index) =>
    ...
    outline = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 340" role="img" aria-label="Saved parcel outline, north up"><path d="${rings}" fill="#d7e9df" fill-rule="evenodd" stroke="#28775c" stroke-width="2"/></svg>`;
  }
...
<section><h2>Source facts</h2><dl><dt>Parcel</dt><dd>${escape(parcel.id)}</dd><dt>Dataset</dt><dd>${escape(manifest.name)}</dd><dt>Source release</dt><dd>${escape(manifest.published)}</dd><dt>INSPIRE ID</dt><dd>${escape(parcel.source?.inspireId ?? 'Not supplied')}</dd><dt>Tenure coverage</dt><dd>Freehold index polygon</dd><dt>Title number</dt><dd>${hasTitleAssessment ? 'Not supplied by INSPIRE; see case title assessments below.' : 'Not established'}</dd><dt>Ownership</dt><dd>${hasTitleAssessment ? 'Not supplied by INSPIRE. Relevant parties and evidence are recorded separately in this case.' : 'Unknown. No ownership evidence loaded.'}</dd></dl></section>
