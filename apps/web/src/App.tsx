...
import { ArrowDownToLine, ArrowLeft, ArrowUpRight, Check, ChevronRight, Database, FileText, Layers, Map, Search, ShieldCheck, X } from 'lucide-react';
...
export function App() {
  ...
  const [view, setView] = useState<'map' | 'review' | 'sources'>('map');
  ...
  async function load(signal?: AbortSignal) {
    ...
    try {
      const [session, data, status] = await Promise.all([
  ...
  useEffect(() => { setResultLimit(100); }, [deferredQuery, filter, interest, view]);
    ...
    try {
      const data = dataset === 'pilot' ? { type: 'FeatureCollection', synthetic: false, manifest,
        ...
        features: parcels.filter(parcel => ids.includes(parcel.id)).map(parcel => ({ type: 'Feature', id: parcel.id, geometry: parcel.geometry,
          properties: { id: parcel.id, source: parcel.source, status: parcelStatus(parcel), links: parcel.links } })),
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <nav aria-label="Main navigation">
        <button className={view === 'map' ? 'active' : ''} onClick={() => setView('map')}><Map size={17} />Explore</button>
        <button className={view === 'review' ? 'active' : ''} onClick={() => setView('review')}><ShieldCheck size={17} />Review<span className="count">{pending}</span></button>
        <button className={view === 'sources' ? 'active' : ''} onClick={() => setView('sources')}><Database size={17} />Data</button>
      </nav>
      <select className="dataset-select" aria-label="Workspace dataset" value={dataset} onChange={event => { setDataset(event.target.value as 'pilot' | 'demo'); setView('map'); }}><option value="pilot">Bristol / INSPIRE</option><option value="demo">Synthetic review demo</option></select>
    </header>
    <div className="demo-banner"><span>{dataset === 'pilot' ? <><strong>Bristol pilot extract</strong> Indicative freehold boundaries. Ownership not established. Not legal title extents.</> : <><strong>Synthetic preview</strong> Fictional parcels, organisations and evidence. No real ownership.</>}</span><span className="mono">{manifest?.published ?? 'BUILD 0.2'}</span></div>
    ...
    {view === 'sources' ? <main className="sources-view">
      ...
      {current && <aside className="detail-panel" aria-label="Parcel evidence">
        ...
        <div className="detail-footer"><button onClick={() => void exportRecords([current.id])}><ArrowDownToLine size={16} />{dataset === 'pilot' ? 'Export parcel GeoJSON' : 'Export synthetic GeoJSON'}</button></div>
      </aside>}
    </main>}
  </div>;
}