...
export function ConsentEditor({ value, onChange, disabled, savedId, savedReady, documents, onUpload, onRequest }: {
  ...
  return <section className="consent-editor" aria-label="Landowner contact and consent">
    ...
    <fieldset disabled={disabled} className="case-fields"><legend>{tab}</legend>
      ...
      {tab === 'Titles' && <>
        {value.titles.map((title, index) => {
          ...
          return <fieldset className="case-record" key={title.id}><legend>Title {index + 1}</legend><div className="case-grid">
      ...
      {tab === 'Parties' && <>
        ...
        {value.parties.map((party, index) => {
          ...
          return <fieldset className="case-record" key={party.id}><legend>Party {index + 1}</legend><div className="case-grid">
      ...
      {tab === 'Requests' && <>
        {value.consents.map((permission, index) => {
          ...
          return <fieldset className="case-record" key={permission.id}><legend>Consent {index + 1}: {effectiveConsentStatus(permission)}</legend><div className="case-grid">
      ...
      {tab === 'Correspondence' && <>
        {value.correspondence.map((entry, index) => {
          ...
          return <fieldset className="case-record" key={entry.id}><legend>Correspondence {index + 1}</legend><div className="case-grid">
